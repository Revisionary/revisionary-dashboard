...
export default {
  ...
  computed: {
    ...
    current() {
      ...
      if (this.$route.name == "revise-deviceID") return "Marc Pridmore > Home"; // Get from Device !!!
  ...
  methods: {
    ...
    async bringPages(projectID) {
      if (this.pages.filter(page => page.project_ID == projectID).length) {
        this.pages = [];
        this.phases = [];
        this.devices = [];
        return false;
      ...
      await this.$axios
        ...
        .then(({ status, data }) => {
          if (status === 200) {
            ...
            this.pages = pages.filter(page => !page.archived && !page.deleted);
    ...
    async bringDevices(phaseID) {
      if (this.devices.filter(device => device.phase_ID == phaseID).length) {
        this.devices = [];
        return false;
      }
